...
export async function getAllPosts(locale) {
  const entries = await fetchGraphQL(
    `query {
      postCollection(order: date_DESC, locale: "${LOCALES[locale]}") {
        items {
          ${POST_GRAPHQL_FIELDS}
        }
      }
...
export async function getAllPostsWithSlug(locale) {
  const entries = await fetchGraphQL(
    `query {
      postCollection(where: { slug_exists: true }, order: date_DESC, locale: "${LOCALES[locale]}") {
        items {
          ${POST_GRAPHQL_FIELDS}
        }
      }
...
export async function getPostAndMorePosts(slug, locale) {
  const entry = await fetchGraphQL(
    `query {
      postCollection(where: { slug: "${slug}" },locale: "${LOCALES[locale]}", limit: 1) {
        items {
          ${POST_DETAIL_GRAPHQL_FIELDS}
        }
      }
  ...
  const entries = await fetchGraphQL(
    `query {
      postCollection(where: { slug_not_in: "${slug}" }, order: date_DESC,locale: "${LOCALES[locale]}",limit: 3) {
        items {
          ${POST_GRAPHQL_FIELDS}
        }
      }
...
export async function getPostsForHome(locale) {
  const entries = await fetchGraphQL(
    `query {
      postCollection( order: date_DESC, locale: "${LOCALES[locale]}", limit: 4) {
        items {
          ${POST_GRAPHQL_FIELDS}
        }
      }
...
export async function getTestimonials(locale) {
  const entries = await fetchGraphQL(
    `query {
      testimonialCollection( locale: "${LOCALES[locale]}") {
        items {
          ${TESTIMONIAL_GRAPHQL_FIELDS}
        }
      }
...
export async function getFaq(locale) {
  const entries = await fetchGraphQL(
    `query {
      faqCollection(locale: "${LOCALES[locale]}"){
        items {
          ${FAQ_GRAPHQL_FIELDS}
        }
      }
...
export async function getNavigation(locale) {
  const entries = await fetchGraphQL(
    `query {
      navigationCollection(order: position_ASC, locale: "${LOCALES[locale]}" ) {
        items {
          ${NAVIGATION_GRAPHQL_FIELDS}
        }
      }
...
export async function getCoverImage(locale) {
  const entries = await fetchGraphQL(
    `query {
      coverCollection(locale: "${LOCALES[locale]}", limit:5) {
        items {
          ${COVER_GRAPHQL_FIELDS}
        }
      }
