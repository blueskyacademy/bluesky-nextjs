...
export function extractTeacherEntries(fetchResponse) {
  const teachers = fetchResponse?.data?.teacherCollection?.items || [];
  return teachers.sort((a, b) => a?.joiningYear < b?.joiningYear).reverse();
...
export function extractStatsEntries(fetchResponse) {
  return fetchResponse?.data?.statCollection?.items;
}
export function extractCoverEntries(fetchResponse) {
  return fetchResponse?.data?.coverCollection?.items?.[0];
}
